 ...
 */
@WebServlet(description = "a simple one", urlPatterns = { "/teacher_timetable" },
initParams={@WebInitParam(name="defaultUser", value="John Doe")}
)
public class teacher_timetable extends HttpServlet {
	...
	String[][] teacher_timetable = 	Teacher.getTeacher(teacher);
	...
	writer.println("<form action='teacherinfo' method='post'>");	
	int a = 8;
	for (int i=0;i<10;i++){
			writer.println("<tr>");
			...
			for(int j=0;j<6;j++){
				...
				else if(teacher_timetable[i][j]==null)
				{
				...
				writer.println( "<td><input type='text' style = \"width: 100%; margin-bottom: 10px; background:rgba(43, 80, 127, 0.94); border: none; outline: none; padding: 5px; color: #fff;  text-shadow: 1px 1px 1px rgba(0,0,0,0.3); border: 1px solid rgba(0,0,0,0.3); border-radius: 4px; box-shadow: inset 0 -5px 45px rgba(100,100,100,0.2), 0 1px 1px rgba(255,255,255,0.2); -webkit-transition: box-shadow .5s ease; -moz-transition: box-shadow .5s ease; -o-transition: box-shadow .5s ease; -ms-transition: box-shadow .5s ease; font-weight:bold; font-size: 14px; transition: box-shadow .5s ease;    \"name='text[" + i + "][" + j + "]' " +  "value = '"+teacher_timetable[i][j]+"'></td>");
				
				}
			}
		}writer.println("</tr></table>");
		
		writer.println("<input type = hidden name = 'teacher' value =  \" "+ teacher +"\">");
